    ...
    ! ===== Phase-1B: λ_mem(z) の z依存性 =====
    ! if (lambda0 > 1e-12_dl) then
        z = 1._dl/a - 1._dl
    ! lambda_mem = this%CP%fde_lambda0 * (1._dl + z)**this%CP%fde_alpha
        lambda_mem = lambda0 * (1._dl + z)**alpha

    ! if (lambda_mem < 0._dl) lambda_mem = 0._dl       ! Clip / Floor

    ! if (this%CP%fde_lambda0 /= 0) then
    !     print*, "FDE ACTIVE lambda_mem =", lambda_mem
    ! end if

    ! grhoa2 に反映
        grhoa2 = grhoa2 * (1._dl + lambda_mem)**2
    ! endif
